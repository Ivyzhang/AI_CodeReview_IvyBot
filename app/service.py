...
class ReviewService:
    ...
    def _process(self, task: ReviewTask) -> None:
        ...
        if not policy.enabled or (
            task.trigger_mode is TriggerMode.AUTOMATIC
            and task.trigger == "synchronize"
            and not policy.auto_review
        ):
            self._supersede(task)
            return
        ...
        if (
            task.trigger_mode is TriggerMode.AUTOMATIC
            and not any(fnmatch.fnmatch(base_ref, pattern) for pattern in policy.include_branches)
        ):
            self._supersede(task)
            return
        if task.trigger_mode is TriggerMode.AUTOMATIC:
            if pr.get("draft") and not policy.review_drafts:
                self._supersede(task)
                return
            if task.trigger_actor_type == "Bot" and not policy.review_bot_prs:
                self._supersede(task)
                return
