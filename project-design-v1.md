...
该模块不能调用模型、读取仓库策略或执行完整 Review；仓库策略、PR 文件和模型上下文统一由 Worker 获取。

### 7.2 Task Store

`TaskStore` 定义任务创建、获取、状态流转、恢复和发布记录协议。V1 使用 SQLite 实现，由单个 Worker 顺序领取 `queued` 任务。

Webhook 事务写入失败时返回 `503`，不记录 Delivery，使 GitHub 后续重试能够重新提交。Worker 持续扫描超过执行超时仍处于 `running` 的任务并恢复为 `queued`；瞬态网络和限流错误最多退避重试两次。因此 V1 提供至少一次任务执行语义，并通过 Review 级幂等避免重复发布。
